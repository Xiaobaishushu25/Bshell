...
import xbss.myterminal.jediterm.terminal.emulator.mouse.TerminalMouseListener;
import xbss.myterminal.jediterm.terminal.model.*;
import xbss.myterminal.jediterm.terminal.model.hyperlinks.LinkInfo;
import xbss.myterminal.jediterm.terminal.ui.hyperlinks.LinkInfoEx;
import xbss.myterminal.jediterm.terminal.ui.input.AwtMouseEvent;
import xbss.myterminal.jediterm.terminal.ui.input.AwtMouseWheelEvent;
...
import xbss.myterminal.jediterm.terminal.util.Pair;
...
public class TerminalPanel extends JComponent implements TerminalDisplay, TerminalActionProvider {
  ...
  public TerminalPanel(@NotNull SettingsProvider settingsProvider, @NotNull TerminalTextBuffer terminalTextBuffer, @NotNull StyleState styleState) {
    ...
    myCursorStyle = mySettingsProvider.getCursorStyle();
  ...
  public void init(@NotNull JScrollBar scrollBar) {
    ...
    addHierarchyListener(new HierarchyListener() {
      ...
      public void hierarchyChanged(HierarchyEvent e) {
        // replace with xbss.myterminal.intellij.util.ui.update.UiNotifyConnector#doWhenFirstShown when merged with intellij
...
//            System.out.println("Error while terminal panel redraw", ex);
  ...
  /**
   *  控制滚动快慢，原来是滚动一下走一行，我乘5之后一次走5行
   * @param k 鼠标滚轮滚动次数 一直是1
   */
  private void moveScrollBar(int k) {
//    myBoundedRangeModel.setValue(myBoundedRangeModel.getValue() + k);
    myBoundedRangeModel.setValue(myBoundedRangeModel.getValue() + 5*k);
  }
...
//      LOG.info("WARNING: Font " + myNormalFont.getName() + " is non-monospaced");
  ...
  /**
   *  这个方法只要终端在视野中且有输入焦点就会每帧调用绘制终端界面、或者有新消息到来(长时间没有消息到来后再来好像就不会调用了)
...
//    System.out.println("绘制");
    ...
    //这一句是我加的，当两个背景都透明后，不加这一句清除的话，会一直叠加越来越模糊，因为背景不透明时会先糊一层背景在渲染字
...
//        myCursor.drawCursor(cursorChar, gfx, inSelection ? selectionStyle : normalStyle);
        /*  这里会根据当前光标有没有被选择来传递不同的样式，我将没有选择时的光标的样式修改为自定义的myCursorStyle */
        myCursor.drawCursor(cursorChar, gfx, inSelection ? selectionStyle : myCursorStyle);
      }
    } finally {
  ...
  // also called from xbss.myterminal.intellij.terminal.JBTerminalPanel
...
//      inverseBg = new java.awt.Color(148, 0, 211);
  ...
  private void drawCharacters(int x, int y, TextStyle style, CharBuffer buf, Graphics2D gfx,
    ...
    /**
     * gfx.setColor(backgroundColor);这里设置的是选中部分的高亮背景色
     */
    ...
    /**
     * gfx.setColor(getStyleForeground(style)); 这个是设置终端中所有字体的颜色
     */
  ...
  private boolean processTerminalKeyPressed(KeyEvent e) {
    ...
    try {
      ...
      //  下面这段是我注掉的，好像是检测是否只按下了Alt键以及后续一些条件，不知道是在干什么的，但是会导致插入一个字符：^[^R ，我就先注掉了
...
//      System.out.println("Error sending pressed key to emulator", ex);
  ...
  private boolean processCharacter(@NotNull KeyEvent e) {
    ...
    //这句是我自己加的，按下ctrl等按键不要滚动到底部 e.getKeyCode() == 27是ESC,按下这几个键会被过滤
//    if (e.isControlDown()||e.isShiftDown()||e.isMetaDown()||e.getKeyCode() == 27)
    if (e.isControlDown()||e.isMetaDown()||e.getKeyCode() == 27)
      return false;
    myTerminalStarter.sendString(new String(obuffer), true);
    if (mySettingsProvider.scrollToBottomOnTyping()) {
      //这里按很多键按下会滚动到底部
//      System.out.println("mySettingsProvider滚动到底部");
      scrollToBottom();
    }
    return true;
  }
