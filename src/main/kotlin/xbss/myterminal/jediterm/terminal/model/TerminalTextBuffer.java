...
import xbss.myterminal.jediterm.terminal.StyledTextConsumer;
import xbss.myterminal.jediterm.terminal.StyledTextConsumerAdapter;
import xbss.myterminal.jediterm.terminal.TextStyle;
import xbss.myterminal.jediterm.terminal.model.TerminalLine.TextEntry;
...
//  private static final Logger LOG = LoggerFactory.getLogger(TerminalTextBuffer.class);
  ...
  /**
   * 我后加的，监听宽度变化用于设置模拟终端的宽度
   */
  public SimpleIntegerProperty myWidthProperty = new SimpleIntegerProperty(-1);
...
//      LOG.error("Attempt to insert negative blank chars number: count:" + count);
  ...
  /**
   *  我自己加的，以textStyle风格写入text
   * @param x
   * @param y
   * @param str
   * @param textStyle
   */
  public void writeStringWithTextStyle(final int x, final int y, @NotNull final CharBuffer str,TextStyle textStyle) {
    writeString(x, y, str, textStyle);
  }
  ...
  public void addLineAtIndex(@NotNull final TerminalLine line,int index) {
    myScreenBuffer.addLinesAtIndex(List.of(line),index);
    fireModelChangeEvent();
  }
